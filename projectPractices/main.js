//const url = "https://superheroapi.com/api/10224877589389880/70";
/*
...
function getJSON(url) {
  ...
  //  View code
  function renderSuperHeroList(ships, shipListElement) {
    ...
    ships.forEach(function (ship) {
      ...
      listItem.innerHTML = `
          <td><a href="${ship.image.url}">${ship.name}</a></td>
          <td>${ship.id}</td>
  ...
  function renderSuperHeroDetails(shipData) {
    console.log(shipData);
  }
  
  // controller code
  function showSuperHeroes(url ) {//= "https://swapi.dev/api/starships/"
    getSuperHeroes(url).then(function (data) {
      console.log(data);
      const results = data.results;
  
      //get the list element
      const shipListElement = document.getElementById("superHeroList");
      renderSuperHeroList(results, shipListElement);
      ...
      if (data.previous) {
        ...
        prev.ontouchend = () => {
          showSuperHeroes(data.previous);
        };
      }
    });
  }
  
  function getSuperHeroDetails(url) {
...
function getJSON(url) {
  ...
  // controller code
  function showShips(url = "https://superheroapi.com/api/10224877589389880/70"){//"https://swapi.dev/api/starships/") {
